...
호출은 전부 `state.mode === "story"` 로 감싼다. 무한모드는 체력·속도·**피해량** 세 축
모두 상한이 없다(`hpMult = 1+i*0.05`, `spdMult = 1+i*0.02`, `dmgMult = 1+i*0.03`,
`core/state.ts` 의 `beginChapter()`) — 언젠가 죽는 게 무한모드의 요점이라 인위적인
천장을 두지 않기로 했다. **피해량 축은 나중에 추가한 것이다** — 처음엔 체력·속도만
올렸는데, 체력 업그레이드를 충분히 쌓으면 받는 피해가 안 늘어서 사실상 안 죽는
문제가 있었다(적이 세지긴 해도 한 대 맞는 피해 자체는 스토리 모드 값 그대로였다).
`Enemy.dmgMult` 를 스폰 시 개체별로 심어서 접촉·투사체 피해 둘 다에 곱한다
(`systems/enemies.ts`) — `ENEMY_DEF` 자체는 스토리 모드와 공유하는 전역 테이블이라
직접 못 바꾸고, 인스턴스에 배율을 얹는 식으로 우회했다.
...
- 무한모드 난이도 곡선: `data/chapters.ts` 의 `endlessChapter()`(적 구성)와
  `core/state.ts` 의 `beginChapter()` 안 `hpMult`/`spdMult`/`dmgMult`
  (파도당 체력·속도·피해량 배율, 셋 다 상한 없음)
...
확정한 기준선이니, **스토리 모드 밸런스는 다시 요청받기 전까지 건드리지 말 것.**

2026-08-15: 무한모드에 `dmgMult` 를 추가했다(위 참고) — `state.mode==="endless"`
일 때만 적용되므로 위 스토리 모드 기준선에는 영향이 없다. 체력만 계속 사는
극단적 케이스(접촉을 1회씩 흉내내며 매 라운드 hp 만 구매)로 헤드리스 확인한 결과
35라운드 근방에서 사망 — 예전엔(dmgMult 없이) 이론상 죽지 않았다.

### 남은 일

1. **`supabase.sql` 재실행 필요 — 이번엔 필수입니다.** `mode` 컬럼 추가, 유니크
   인덱스/트리거를 `(nick, mode)` 기준으로 재구성, `stage`/`score`/`kills`/`run_time`
   상한을 전부 모드별 조건부로 재구성하는 스키마 변경이 들어 있다. 실행 전에는
   무한모드에서 어느 정도 성장한 기록은(예: 648,110점 · 킬 5,478) 등록이 실패한다
   (첫 버전은 `stage_range` 만 넓히고 `score_range`/`kills_range` 를 그대로 둔 채라
   무한모드 등록이 400 으로 막혔었다 — 이번에 같이 고쳤다). (2026-08-15 기준 미실행)
